...
In addition to the main project directory, each project is associated with a “Publish” directory that can be used to share results with collaborators. The Publish directory could be a Dropbox, file server or web server directory. See figure 1 for the Create Project tab. Projects can be created within the R console using the ```plant.tree``` function ```plant.tree("MyProject","Path to project","Path to publish directory")```. A list of all projects can be obtained using the ```get_orchard()``` command. The function ```uproot_tree(project.id)``` will remove the project from the orchard file, which dissociates the project from adapr, but
does not remove the project from the file system. The function ```redirect.tree()``` tree will edit the project's location in the file system.
...
`create_source_file_dir` and packages needed for the project will be automatically installed. The functions ```add_package(project.id,package,install.command,specific)``` and ```remove_package(project.id,package.name)``` will add or remove an R package from a project. To install all packages for a project, use the function
