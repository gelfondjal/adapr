---
title: "adapr Package Instructions"
...
In addition to the main project directory, each project is associated with a “Publish” directory that can be used to share results with collaborators. The Publish directory could be a Dropbox, file server or web server directory. See figure 1 for the Create Project tab. 
...
This code snippet creates a file in the results directory of the project called histogram.png with a histogram of 100 standard normal variables. Another useful function is the following

```
LoadedObject <- Load.branch("read_data.R/mydata.Rdata"")
```

This function reads in an R data object with the `.Rdata` file extension written by another program. This function searches through all results from the project to identify file, and returns the corresponding R object (assigned to in LoadedObject example). The function follows Write within another R script (read_data.R) to avoid reading the same file twice and to pass preprocessed data from one R script to another. Each R script ends with a footer
```
...
R packages needed for each R script can be added in the usual manner. However, the ADAPR app facilitates loading libraries and tracking which library are used by creating a common library file that contains a list of libraries used by project. Each library is specified therein is as specific to a particular R script or not. If the library is not labeled as specific to an R script is will be automatically loaded for each R script in the project. These libraries are managed with the Programs tab of the app. Figure 2. Library management is needed for reproducibility. The libraries need not be specified by the app, but can be included in the R scripts header prior to the creation of `source_info`. The function ` create_source_file_dir` will detect and record libraries that are already loaded.
