...
does not remove the project from the file system. The function ```reidentify.project()``` tree will edit the location for which adapr will associate with the project.
...
```plot.project("adaprHome")  # shows the DAG of the project with synchronization status```.
...
`install_project_packages(project.id)`. The package versions are recorded within the program support directories.
...
lists for an R script can overwhelm the visual space of these graphics.
...
```open.pubresults(project.id) # Browses publication table for editing```
...
There are some convenience functions that can be used within the R console if there is a `source_info` object in the R environment. These can be initiated without specifying the project, if ```set.project(project.id)``` is used to indicate the working project using the "adaprProject" R option withing ```options```. 
